window.function = async function(api_key, thread_id, run_id, limit, order, after, before, include) {
    // Validate API Key
    if (!api_key.value) {
    ...
    // API endpoint URL
    const apiUrl = `https://api.openai.com/v1/threads/${thread_id.value}/runs/${run_id.value}/steps?${queryParams.toString()}`;
